...
def tokenizer(vocab_file: str = None) -> run.Config[pl.LightningModule]:
    ...
    if vocab_file:
        ...
        return run.Config(
            get_nmt_tokenizer,
            library='huggingface',
            model_name="nvidia/Nemotron-H-4B-Base-8K",
            use_fast=True,
        )
...
def model(vocab_file: str = None) -> run.Config[pl.LightningModule]:
    """
    Factory function to create a NemotronH Hybrid 4B model configuration.
    Returns:
        run.Config[pl.LightningModule]: Configuration for the NemotronH Hybrid 4B model.
    Examples:
        CLI usage:
            $ nemo llm pretrain model=nemotronh_4b ...
        Python API usage:
            >>> model_config = model()
            >>> print(model_config)
    """
    return run.Config(
        llm.MambaModel,
        config=run.Config(llm.NemotronHConfig4B),
...
def trainer(
    ...
    """
    Configure the NeMo Lightning Trainer for NemotronH Hybrid 4B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain trainer=nemotronh_4b ...
        Python API usage:
...
def pretrain_recipe(
    ...
    """
    Create a pre-training recipe for NemotronH Hybrid 4B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory nemotronh_4b
            $ nemo llm pretrain --factory "nemotronh_4b(num_nodes=32, name='my_pretrain')"
        Python API usage:
            >>> recipe = pretrain_recipe(name="nemotronh_4b_pretrain", num_nodes=32)
            >>> print(recipe)
    """
...
@run.cli.factory(target=finetune, name=NAME)
def finetune_recipe(
    resume_path: str = "nemotronh-4b-pretrain",
    vocab_file: str = None,
    dir: Optional[str] = None,
    name: str = "default",
    num_nodes: int = 8,
    ...
    """
    Create a fine-tuning recipe for NemotronH Hybrid 4B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory nemotronh_4b
        Python API usage:
            >>> recipe = finetune_recipe(name="nemotronh_4b_finetune", num_nodes=32)
            >>> print(recipe)
    Note:
        This recipe uses the SQuAD dataset for fine-tuning.
        For converting an SSM pytorch checkpoint, use the following line of python code:
        llm.MambaModel(llm.NemotronHConfig4B(), tokenizer=tokenizer(vocab_file=vocab_file)).import_ckpt(
            path="pytorch://ABSOLUTE_PATH_TO_CKPT/your_pytorch_state_dict_file",
            model_config=llm.NemotronHConfig4B())
