...
from nemo.collections.speechlm2 import HybridSALMTDT
...
def main(cfg: SalmTdtEvalConfig):
        ...
        if cfg.task_type == "salm":
            ...
            for hyp in hypotheses:
                if hasattr(hyp, 'text'):
                    ...
                    text = str(hyp)
                batch_hyps.append(normalizer(text.strip()))
            
        elif cfg.task_type == "both":
            # Evaluate using both heads and combine results
            ...
            salm_hyps = [
                normalizer(model.tokenizer.ids_to_text(parse_hyp(ans, eos_tokens)).strip()) for ans in salm_answer_ids
            ]
            ...
            # Combine results with weight
            batch_hyps = []
            for salm_hyp, tdt_hyp in zip(salm_hyps, tdt_hypotheses):
                tdt_text = tdt_hyp.text if hasattr(tdt_hyp, 'text') else str(tdt_hyp)
                tdt_text = normalizer(tdt_text.strip())
                # Combine with weight (you can adjust this logic)
                combined = f"{salm_hyp} [TDT: {tdt_text}]"
                batch_hyps.append(combined)
    ...
    logging.info(f"Overall WER: {wer:.2%} [ins={nins:.2%} del={ndel:.2%} sub={nsub:.2%}]")
