...
from nemo.collections.speechlm2 import HybridSALMTDT, SALMDataset
from nemo.core.config import hydra_runner
from nemo.utils import logging


@dataclass
class SalmTdtEvalConfig:
    # Support both checkpoint path and pretrained name
    ...
    # TDT-specific parameters
    task_type: str = "salm"  # "salm", "tdt", or "both"
    tdt_weight: float = 0.3


@hydra_runner(config_name="SalmTdtEvalConfig", schema=SalmTdtEvalConfig)
def main(cfg: SalmTdtEvalConfig):
    logging.info(f"Hydra config:\n{OmegaConf.to_yaml(cfg)}")

    # Load model from checkpoint or pretrained name
    if cfg.checkpoint_path is not None:
        logging.info(f"Loading model from checkpoint: {cfg.checkpoint_path}")
        model = HybridSALMTDT.load_from_checkpoint(cfg.checkpoint_path).eval().to(getattr(torch, cfg.dtype)).to(cfg.device)
    elif cfg.pretrained_name is not None:
        logging.info(f"Loading model from pretrained name: {cfg.pretrained_name}")
        model = HybridSALMTDT.from_pretrained(cfg.pretrained_name).eval().to(getattr(torch, cfg.dtype)).to(cfg.device)
    else:
        raise ValueError("Either 'checkpoint_path' or 'pretrained_name' must be provided")
    ...
    for batch_idx, batch in enumerate(dloader):
        ...
        if cfg.task_type == "salm":
            # Generate using SALM head
            answer_ids = model.generate_salm(
        ...
        elif cfg.task_type == "tdt":
            # Transcribe using TDT head
            hypotheses = model.transcribe_tdt(
                audios=batch["audios"].to(model.device, non_blocking=True),
                audio_lens=batch["audio_lens"].to(model.device, non_blocking=True),
            )
            
            # Convert TDT hypotheses to text
            batch_answers = []
            batch_num_answer_tokens = []
            for hyp in hypotheses:
                if hasattr(hyp, 'text'):
                    ...
                    text = str(hyp)
                batch_answers.append(text)
                batch_num_answer_tokens.append(len(text.split()))
            
            # For TDT, we don't have input contexts in the same way
            batch_contexts = ["TDT Transcription"] * len(batch_answers)
            
        elif cfg.task_type == "both":
            # Generate using both heads and combine results
            # First, get SALM generation
            salm_answer_ids = model.generate_salm(
                prompts=batch["input_ids"].to(model.device, non_blocking=True),
                audios=batch["audios"].to(model.device, non_blocking=True),
                audio_lens=batch["audio_lens"].to(model.device, non_blocking=True),
                generation_config=GenerationConfig(
            ...
            salm_answer_ids = salm_answer_ids.cpu()
            salm_answers = [model.tokenizer.ids_to_text(parse_hyp(ans, eos_tokens)) for ans in salm_answer_ids]
            
            # Then, get TDT transcription
            tdt_hypotheses = model.transcribe_tdt(
                audios=batch["audios"].to(model.device, non_blocking=True),
                audio_lens=batch["audio_lens"].to(model.device, non_blocking=True),
            )
            ...
            batch_answers = []
            batch_num_answer_tokens = []
            for salm_ans, tdt_hyp in zip(salm_answers, tdt_hypotheses):
                tdt_text = tdt_hyp.text if hasattr(tdt_hyp, 'text') else str(tdt_hyp)
                # Combine with weight
                combined = f"SALM: {salm_ans}\nTDT: {tdt_text}"
                batch_answers.append(combined)
                batch_num_answer_tokens.append(len(combined.split()))
            
            batch_contexts = [model.tokenizer.ids_to_text(example) for example in batch["input_ids"]]
        
        else:
            raise ValueError(f"Invalid task_type: {cfg.task_type}. Must be 'salm', 'tdt', or 'both'")
